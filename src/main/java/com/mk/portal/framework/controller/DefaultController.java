...
checkUser(request);
	...
	public String blankRequest(
			...
			HttpServletRequest request) {
		checkUser(request);
		return redirectToLogin(model,new PageIdentifier("","","", ""));
	}

	private String redirectToLogin(Model model, PageIdentifier pageIdentifier) {
	...
	private String defaultSitePages(Model model, PageIdentifier pageIdentifier) {
		...
		if(!pageIdentifier.getTopicId().equals("")){
			...
			if(!pageIdentifier.getSubTopicId().equals("")){
				pagepath= pagepath+BLANK_URL+pageIdentifier.getSubTopicId();
